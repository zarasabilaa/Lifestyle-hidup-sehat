...
public class EditMakanSehatForm extends JFrame {
    ...
    public EditMakanSehatForm(int idTips, String namaMakanan, String deskripsi, byte[] gambarAwal) {
        this.idTips = idTips;
        setTitle("Edit Makan Sehat");
        setSize(500, 500);
        ...
        tfJudul = new JTextField(namaMakanan);
        tfJudul.setBounds(140, 30, 300, 25);
        add(tfJudul);

        JLabel lblIsi = new JLabel("Deskripsi:");
        lblIsi.setBounds(30, 70, 100, 25);
        add(lblIsi);

        taIsi = new JTextArea(deskripsi);
    ...
    private void simpanEdit() {
        ...
        try {
            ...
            String sql = "UPDATE makan_sehat SET nama_makanan=?, deskripsi=?, gambar=? WHERE id=?";
            ...
            if (updated > 0) {
                JOptionPane.showMessageDialog(this, "Makan Sehat berhasil diperbarui.");
                new MakanSehatPage();
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Gagal memperbarui Makan Sehat.");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
